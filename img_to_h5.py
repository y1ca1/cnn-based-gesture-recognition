...
# 图片转h5文件
def image_to_h5():
    dirs = os.listdir("/home/caiyi/PycharmProjects/gesture_MP/GestNet")
    Y = []  # label
    X = []  # data
    print(len(dirs))
    for filename in dirs:
        ...
        Y.append(label)
        im = Image.open("/home/caiyi/PycharmProjects/gesture_MP/GestNet//{}".format(filename)).convert('RGB')
        mat = np.asarray(im)  # image 转矩阵
        X.append(mat)

    file = h5py.File("/home/caiyi/PycharmProjects/gesture_MP/dataset2/data.h5", "w")
